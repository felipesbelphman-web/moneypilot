...
import { localizeCopy, localizeData } from "@/i18n/localize-copy";

const iconRoot = "/moneypilot/icons/rawCategories";
...
export default function CategoriesPage() {
  ...
  const metrics = localizeData(language, rawMetrics);
  const categories = localizeData(language, rawCategories);
  const rules = localizeData(language, rawRules);
  const benefits = localizeData(language, rawBenefits);
          ...
          <section className="absolute left-[231px] top-[111px] h-[800px] w-[1164px] overflow-hidden rounded-[38px] border border-[#28313B]/16 bg-[#0D1117]/50 px-[32px] py-[16px] shadow-[0_22px_42px_rgba(0,0,0,0.45)] backdrop-blur-[20px]">
            <div className="flex h-[767.67px] w-[1098px] flex-col gap-[12px]">
              <div className="flex h-[36.75px] shrink-0 items-center gap-[18.375px]">
                ...
                <span className="money-pilot-wordmark font-[Poppins] text-[21.44px] font-medium leading-[26.031px] tracking-[0.4288px]">MoneyPilot</span>
              </div>

              <header className="flex h-[57.92px] shrink-0 items-center justify-between">
                <div className="flex h-[52px] w-[520px] flex-col justify-center gap-[4px]"><h1 className="text-[23.168px] font-semibold leading-none">{tr("Categorias")}</h1><p className="text-[13.2px] text-[#9CA6B2]">{tr("Gerencie categorias, cores e regras automáticas para seus gastos.")}</p></div>
                <div className="flex h-[52px] w-[220px] items-center justify-end gap-[16px]"><button type="button" className="h-[38px] w-[146px] rounded-[19px] bg-[#3B82F6] text-[10px] font-semibold">{tr("+ Nova categoria")}</button><Image src="/moneypilot/dashboard-avatar.png" alt={tr("Avatar do usuário")} width={52} height={52} className="h-[52.128px] w-[52.128px] rounded-full" /></div>
              </header>

              <section aria-label={tr("Resumo das categorias")} className="flex h-[100px] shrink-0 gap-[12px]">
              ...
              <section className="flex h-[260px] shrink-0 gap-[12px]">
                <article className={`${cardClass} relative h-[260px] w-[677px] shrink-0`}>
                  <h2 className="absolute left-[11px] top-[9px] text-[15px] font-semibold">{tr("Lista de categorias")}</h2><button type="button" className="absolute left-[552px] top-[7px] h-[24px] w-[112px] rounded-[12px] bg-[#3B82F6] text-[8.5px] font-semibold">{tr("+ Nova categoria")}</button>
                  <div className="absolute left-[11px] top-[39px] grid h-[22px] w-[653px] grid-cols-[228px_46px_321px_58px] items-center rounded-[7px] bg-[rgba(25,33,44,0.78)] px-[8px] text-[8.2px] font-semibold text-[#7F8996]"><span>{tr("Categoria")}</span><span>{tr("Cor")}</span><span>{tr("Estabelecimentos")}</span><span>{tr("Ações")}</span></div>
                  <div className="absolute left-[11px] top-[61px] w-[653px]">{categories.map((category, index) => <div key={category.name} className={`grid h-[21px] grid-cols-[228px_46px_321px_58px] items-center border-b border-[#28313B]/70 px-[6px] text-[8px] ${index === 1 ? "bg-[#3B82F6]/6" : ""}`}><span className="flex items-center gap-[7px] text-[8.8px] font-semibold"><i className="flex h-[18px] w-[18px] items-center justify-center rounded-[6px]" style={{ backgroundColor: `${category.color}24` }}><Glyph name={category.icon} size={12} color={category.color} /></i>{category.name}</span><i className="h-[7px] w-[7px] rounded-full" style={{ backgroundColor: category.color }} /><span className="text-[#9CA6B2]">{category.merchants}</span><span className="flex items-center justify-end gap-[8px]"><button type="button" aria-label={`${tr("Editar")} ${category.name}`}><Glyph name="edit" size={11} color="#64707D" /></button><button type="button" aria-label={`${tr("Mais opções para")} ${category.name}`}><Glyph name="more" size={12} color="#64707D" /></button></span></div>)}</div>
                </article>

                <article className={`${cardClass} relative h-[260px] w-[409px] shrink-0`}>
                  <h2 className="absolute left-[10px] top-[9px] text-[15px] font-semibold">{tr("Regras automáticas")}</h2><button type="button" className="absolute right-[10px] top-[7px] h-[24px] w-[58px] rounded-[12px] bg-[#3B82F6] text-[8px] font-semibold">{tr("+ Nova regra")}</button>
                  <div className="absolute left-[10px] top-[39px] grid h-[22px] w-[389px] grid-cols-[98px_18px_205px_68px] items-center rounded-[7px] bg-[rgba(25,33,44,0.78)] px-[7px] text-[7.8px] font-semibold text-[#7F8996]"><span>{tr("Estabelecimento")}</span><span /><span>{tr("Mapeamento")}</span><span>{tr("Ações")}</span></div>
                  <div className="absolute left-[10px] top-[61px] w-[389px]">{rules.map((rule) => <div key={rule.merchant} className="grid h-[26px] grid-cols-[98px_18px_205px_68px] items-center border-b border-[#28313B]/70 px-[7px] text-[8px]"><span className="flex items-center gap-[6px] font-semibold"><i className="flex h-[17px] w-[17px] items-center justify-center rounded-[5px] bg-[#3B82F6]/15"><Glyph name={rule.icon} size={11} color="#60A5FA" /></i>{rule.merchant}</span><Glyph name="arrow-right" size={10} color="#64707D" /><span><i className="inline-flex h-[16px] items-center rounded-[8px] px-[9px] not-italic" style={{ color: rule.color, backgroundColor: `${rule.color}18` }}>{rule.category}</i></span><span className="flex justify-end gap-[8px]"><button type="button" aria-label={`${tr("Editar")} ${rule.merchant}`}><Glyph name="edit" size={11} color="#64707D" /></button><button type="button" aria-label={`${tr("Mais opções para")} ${rule.merchant}`}><Glyph name="more" size={12} color="#64707D" /></button></span></div>)}</div>
                </article>
              </section>
              ...
              <section className={`${cardClass} relative h-[163px] shrink-0`}>
                <h2 className="absolute left-[11px] top-[9px] text-[13px] font-semibold">{tr("Editar categoria")}</h2>
                ...
                <label className="absolute left-[191px] top-[34px] w-[130px] text-[7.8px] font-semibold text-[#7F8996]">{tr("Ícone")}<span className="mt-[4px] flex h-[34px] items-center gap-[7px] rounded-[9px] border border-[#28313B] bg-[#080B0F]/34 px-[9px] text-[9px] font-normal text-[#F5F7FA]"><Glyph name="cart" size={14} color="#22C55E" />{tr("Carrinho")}</span></label>
                <div className="absolute left-[821px] top-[34px] h-[49px] w-[264px]"><strong className="text-[8px]">{tr("Categoria padrão")}</strong><div className="mt-[8px] flex items-center gap-[10px]"><button type="button" role="switch" aria-checked={isDefault} onClick={() => setIsDefault((value) => !value)} className={`relative h-[20px] w-[34px] rounded-[10px] ${isDefault ? "bg-[#22C55E]" : "bg-[#28313B]"}`}><span className={`absolute top-[3px] h-[14px] w-[14px] rounded-full bg-white transition-[left] ${isDefault ? "left-[17px]" : "left-[3px]"}`} /></button><span className="text-[7.7px] text-[#9CA6B2]">{tr("Usar como padrão para novos estabelecimentos.")}</span></div></div>
                <div className="absolute left-[11px] top-[93px] h-px w-[1074px] bg-[#28313B]/70" /><h3 className="absolute left-[11px] top-[103px] text-[8.6px] font-semibold">{tr("Estabelecimentos vinculados")}</h3>
                <div className="absolute left-[11px] top-[123px] flex gap-[8px]">{["LIDL", "Dunnes", "Centra"].map((merchant) => <span key={merchant} className="flex h-[24px] items-center gap-[6px] rounded-[12px] bg-[#22C55E]/9 px-[7px] text-[7.8px] font-semibold text-[#B7BEC8]"><Glyph name="store" size={12} color="#22C55E" />{merchant}<button type="button" aria-label={`${tr("Remover")} ${merchant}`}><Glyph name="close" size={10} color="#64707D" /></button></span>)}<button type="button" className="flex h-[24px] items-center rounded-[9px] border border-[#28313B] bg-[#080B0F]/28 px-[11px] text-[7.6px] font-semibold text-[#60A5FA]">{tr("+ Adicionar estabelecimento")}</button></div>
                <div className="absolute right-[13px] top-[119px] flex gap-[10px]"><button type="button" className="h-[28px] w-[74px] rounded-[14px] border border-[#28313B] bg-[#080B0F]/28 text-[8.4px] font-semibold text-[#9CA6B2]">{tr("Cancelar")}</button><button type="button" className="h-[28px] w-[92px] rounded-[14px] bg-[#3B82F6] text-[8.4px] font-semibold">{tr("Salvar alterações")}</button></div>
              </section>
            </div>
          </section>
        </div>
